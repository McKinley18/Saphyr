  ...
  return (
    <div className="income-page">
      ...
      <div className="income-grid-layout">
        ...
        <div className="workflow-column">
          ...
          <div className={`workflow-step ${activeStep === 1 ? 'focused' : 'collapsed'}`}>
            <div className="step-indicator" onClick={() => setActiveStep(1)}>1</div>
            <section className="card" onClick={() => activeStep !== 1 && setActiveStep(1)} style={{ borderLeft: '5px solid var(--primary)' }}>
              <div className="step-header">
                <h3 className="centered-title">EARNINGS</h3>
                {activeStep !== 1 && <span className="step-summary-label">${safeFormat(taxEstimate?.annual_salary)}/yr</span>}
              </div>
                  ...
                  <form onSubmit={onSaveProfile} className="workflow-form" style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
                    {isHourly ? (
                      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                        <div className="form-group"><label>Hourly Rate ($)</label><input type="number" step="0.01" value={hourlyRate || ''} placeholder="0.00" onChange={e => setHourlyRate(parseFloat(e.target.value) || 0)} /></div>
                        <div className="form-group"><label>Hours / Week</label><input type="number" value={hoursPerWeek || ''} placeholder="40" onChange={e => setHoursPerWeek(parseInt(e.target.value) || 0)} /></div>
                      </div>
                    ) : (
                      <div className="form-group"><label>Annual Gross Salary ($)</label><input type="number" value={annualGross || ''} placeholder="0.00" onChange={e => setAnnualGross(parseFloat(e.target.value) || 0)} /></div>
                    )}
          ...
          <div className={`workflow-step ${activeStep === 2 ? 'focused' : 'collapsed'}`}>
            <div className="step-indicator" onClick={() => setActiveStep(2)}>2</div>
            <section className="card" onClick={() => activeStep !== 2 && setActiveStep(2)} style={{ borderLeft: '5px solid var(--warning)' }}>
              <div className="step-header">
                <h3 className="centered-title">DEDUCTIONS</h3>
                {activeStep !== 2 && <span className="step-summary-label">-{safeFormat((taxEstimate?.deduction_401k + taxEstimate?.total_pre_tax_deductions) / 12)}/mo</span>}
              </div>
                ...
                <div className="step-content-expanded">
                  <div className="form-group" style={{ marginBottom: '30px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                      <input type="number" value={pct401k || ''} placeholder="0%" onChange={e => setPct401k(parseFloat(e.target.value) || 0)} />
                      <button type="button" onClick={onSaveProfile} className="warning-btn">SET</button>
                    </div>
                  </div>
                  ...
                  <div className="item-list">
                    ...
                    {(savedSalary?.custom_deductions || []).map((d: any) => (
                      <div key={d.id} className="workflow-item">
                        <div>
                          <span>{d.name}</span>
                          <div style={{ fontSize: '0.6rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>{d.frequency}</div>
                        </div>
                        <div className="item-right">
                          <span className="negative">-${safeFormat(d.amount)}</span>
                          <button type="button" onClick={async () => { await deleteDeduction(d.id); loadData(); }} className="remove-btn">&times;</button>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <form onSubmit={onAddDeduction} className="add-item-form">
                    <div className="form-group"><label style={{ fontSize: '0.65rem' }}>New Pre-Tax (Health, HSA)</label><input placeholder="Obligation Name" value={newDeduction.name} onChange={e => setNewDeduction({...newDeduction, name: e.target.value})} /></div>
                    <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '15px', marginTop: '15px' }}>
                      <input type="number" placeholder="Amount $" value={newDeduction.amount} onChange={e => setNewDeduction({...newDeduction, amount: e.target.value})} />
                      <select value={newDeduction.frequency} onChange={e => setNewDeduction({...newDeduction, frequency: e.target.value})} style={{ fontSize: '0.75rem' }}>
                        <option value="monthly">Monthly</option>
                        <option value="bi-weekly">Bi-Weekly</option>
                        <option value="weekly">Weekly</option>
                      </select>
                    </div>
                    <button type="submit" className="add-btn" style={{ width: '100%', marginTop: '15px' }}>ADD DEDUCTION</button>
                  </form>
                </div>
              )}
            </section>
          ...
          <div className={`workflow-step ${activeStep === 3 ? 'focused' : 'collapsed'}`}>
            <div className="step-indicator" onClick={() => setActiveStep(3)}>3</div>
            <section className="card" onClick={() => activeStep !== 3 && setActiveStep(3)} style={{ borderLeft: '5px solid var(--success)' }}>
              <div className="step-header">
                <h3 className="centered-title">OTHER INCOME</h3>
                {activeStep !== 3 && <span className="step-summary-label">+${safeFormat(totalOtherIncome)}/mo</span>}
              </div>
                  ...
                  <div className="item-list">
                    ...
                    {(incomeSources || []).map(src => (
                      <div key={src.id} className="workflow-item">
                        <div>
                          <span>{src.name} {src.is_taxed && <small className="taxed-label">(Taxed)</small>}</span>
                          <div style={{ fontSize: '0.6rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>{src.frequency}</div>
                        </div>
                        <div className="item-right">
                          <span className="positive">+${safeFormat(src.amount)}</span>
                          <button type="button" onClick={async () => { await deleteIncomeSource(src.id); loadData(); }} className="remove-btn">&times;</button>
                        </div>
                      </div>
                    ))}
                  </div>
                  <form onSubmit={onAddSource} className="add-item-form success">
                    <input placeholder="Source Name" value={newSource.name} onChange={e => setNewSource({...newSource, name: e.target.value})} style={{ marginBottom: '15px' }} />
                    <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                      <input type="number" placeholder="Amount $" value={newSource.amount} onChange={e => setNewSource({...newSource, amount: e.target.value})} />
                      <select value={newSource.frequency} onChange={e => setNewSource({...newSource, frequency: e.target.value})} style={{ fontSize: '0.75rem' }}>
                        <option value="monthly">Monthly</option>
                        <option value="bi-weekly">Bi-Weekly</option>
                        <option value="weekly">Weekly</option>
                      </select>
                    </div>
                    <button type="submit" className="add-btn-success" style={{ width: '100%', marginTop: '15px' }}>ADD SOURCE</button>
                  </form>
                </div>
              )}
            </section>
          ...
          <section className={`card sticky-summary-v2 ${isPulsing ? 'pulse-glow' : ''}`}>
            ...
            <div className="summary-details-v2">
              ...
              <div className="tax-module-v2">
                <div className="tax-header-v2">
                  <span className="label negative">COMBINED TAXES</span>
                  <button type="button" onClick={() => { setUseManualTax(!useManualTax); onSaveProfile(new Event('submit') as any); }} className="override-btn-v2">{useManualTax ? 'MANUAL' : 'AUTO'}</button>
                </div>
                {useManualTax ? (
                  ...
                  <div className="tax-breakdown-v2">
                    <div className="tax-main-v2 negative">-${safeFormat((taxEstimate?.estimated_tax + taxEstimate?.state_tax) / 12)}</div>
                    <div className="tax-sub-v2">Fed: -${safeFormat(taxEstimate?.estimated_tax / 12)} • State ({localState || taxEstimate?.state}): -${safeFormat(taxEstimate?.state_tax / 12)}</div>
                  </div>
                )}
              </div>
              ...
              <div className="final-result-box-v2">
                <div className="result-label-v2">VERIFIED MONTHLY NET</div>
                <div className="result-value-v2 currency positive">${safeFormat(totalVerifiedNetMonthly)}</div>
                <div className="annual-projection-v2">≈ ${safeFormat(totalVerifiedNetMonthly * 12)} ANNUALLY</div>
              </div>

              <div className="specs-toggle-container-v2">
                <button onClick={() => setIsSpecsExpanded(!isSpecsExpanded)} className="specs-toggle-btn-v2">{isSpecsExpanded ? 'HIDE TECH SPECS' : 'SHOW TECH SPECS'}</button>
                {isSpecsExpanded && (
      ...
      <style>{`
        ...
        .step-indicator { position: absolute; left: 0; top: 15px; width: 40px; height: 40px; border-radius: 50%; background: var(--bg); border: 2px solid var(--primary); color: var(--text); display: flex; align-items: center; justify-content: center; fontWeight: 900; font-size: 1.1rem; box-shadow: 0 0 15px var(--accent-glow); flex-shrink: 0; cursor: pointer; }
        ...
        .mode-switcher button.active { background: var(--primary); color: white; border-color: var(--primary); font-weight: 900; }
        ...
        .add-btn { width: auto; background: rgba(255,255,255,0.1); }
